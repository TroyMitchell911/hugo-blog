...
补丁版 release 在 [TroyMitchell911/plano](https://github.com/TroyMitchell911/plano/releases/tag/v0.4.22-retry-failover)。
...
```
bin/
  brightstaff          # 补丁版控制面二进制（从源码编译）
  envoy                # Envoy v1.37.0 数据面
plugins/
  llm_gateway.wasm     # 补丁版 LLM 网关 WASM filter（从源码编译）
  prompt_gateway.wasm  # 补丁版 prompt 网关 WASM filter（从源码编译）
checksums.sha256
```

> **重要：** 两个 WASM filter（`llm_gateway.wasm` 和 `prompt_gateway.wasm`）必须是本 release 中从源码编译的版本。重试和故障转移的路由逻辑在 WASM filter 内部——如果 `planoai` 下载了官方 WASM 而不是使用 `~/.plano/plugins/` 中的补丁版，重试和故障转移将静默失效。
>
> 运行 `planoai up` 后，检查启动输出。你应该看到：
> ```
...
> ```
> 如果看到 `Downloading WASM plugins`，说明补丁版 WASM 没有被使用——请确认 tarball 已正确解压到 `~/.plano/`，且 `~/.plano/plugins/` 下存在两个 `.wasm` 文件。
